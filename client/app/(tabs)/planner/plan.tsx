...
export default function PlanScreen() {
  ...
  const clientId = process.env.EXPO_PUBLIC_GOOGLE_CLIENT_ID as
    | string
    | undefined;
  const redirectUri = process.env.EXPO_PUBLIC_GOOGLE_REDIRECT_URI as
    | string
    | undefined;
  ...
  const pushPlan = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const successMessage = `${eventsCreated} event${
          eventsCreated !== 1 ? "s" : ""
        ...
        Alert.alert(
          "Error",
          data?.message || "Failed to push plan to Google Calendar"
        );
        if (
          data?.errors &&
  ...
  const toggleCompletion = async (itemId?: string, nextValue?: boolean) => {
    ...
    setItems((prev) =>
      prev.map((it) =>
        it._id === itemId ? { ...it, completed: nextValue } : it
      )
    );
    try {
  ...
  const disconnectGoogle = async () => {
    try {
      ...
      if (!resp.ok) {
        const errBody = await resp.json().catch(() => ({}));
        throw new Error(
          errBody.message || "Failed to disconnect Google Calendar"
        );
      }
      setConnected(false);
      ...
      Alert.alert(
        "Error",
        err?.message || "Could not disconnect Google Calendar"
      );
    }
  };
                ...
                <View key={e._id || idx} style={styles.entryRow}>
                  <TouchableOpacity
                    style={[
                      ...
                      e.completed ? styles.completedBackground : null,
                    ]}
                  ...
                  >
                    <View
                      style={[
                        styles.checkbox,
                        e.completed && styles.checkboxChecked,
                      ]}
                    >
                      {e.completed ? (
                    ...
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[
                          styles.entryTime,
                          e.completed && styles.completedText,
                        ]}
                      >
      ...
      <View style={styles.footer}>
        <View style={styles.statusCard}>
          <View
            style={[
              styles.statusDot,
              { backgroundColor: connected ? "#77C272" : "#ff9800" },
            ]}
          />
        ...
        {connected ? (
          <>
            <TouchableOpacity
              style={[styles.pushBtn, styles.disconnectBtn]}
            ...
            <TouchableOpacity
              style={[
                styles.pushBtn,
                { marginTop: 10, opacity: syncing ? 0.7 : 1 },
              ]}
              onPress={pushPlan}
              disabled={syncing}
            >
              ...
              <Text style={styles.pushText}>
                {syncing ? "Pushing…" : "Push Plan to Google"}
              </Text>
            </TouchableOpacity>
          </>
        ) : (
